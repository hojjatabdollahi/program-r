...
class TemplateGraphBotTests(TemplateGraphTestClient):
    ...
    def test_extension_as_attrib(self):
        template = ET.fromstring("""
			<template>
				<extension path="programrtest.parser.template.graph_tests.test_extension.TestExtension">
				1 2 3
				</extension>
			</template>
			""")
    ...
    def test_extension_as_child(self):
        template = ET.fromstring("""
            <template>
                <extension>
                    <path>programrtest.parser.template.graph_tests.test_extension.TestExtension</path>
                    1 2 3
                </extension>
            </template>
            """)
