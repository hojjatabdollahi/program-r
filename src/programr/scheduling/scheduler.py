...
def scheduled(name, userid, clientid, action, text):
    ProgramrScheduler.scheduled_event(name, userid, clientid, action, text)


def scheduler_listener(event):
    ProgramrScheduler.listener_event(event)


class ProgramrScheduler(object):
    ...
    def register_schedulder(self, scheduler, name):
        if name in ProgramrScheduler.schedulers:
            YLogger.error(None, "Scheduler with name [%s] already registered", name)
        else:
            ProgramrScheduler.schedulers[name] = scheduler
    ...
    def scheduled_event(name, userid, clientid, action, text):
        ...
        if name in ProgramrScheduler.schedulers:
            scheduler = ProgramrScheduler.schedulers[name]
